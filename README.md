...
- **Pluggable runner, other agents/harnesses planned.** Claude is the only shipping adapter today, but support for other coding agents and harnesses (Qwen Code, OpenHands, a local script, or whatever's next) is a design goal, not an afterthought — anything fitting the `runner.Runner` interface (ADR-0007) plugs in without touching the core state machine.
...
|---|---|---|
| **Interactive coding assistants (session-driven)** | Claude Code and similar terminal coding assistants | Not a competitor — a substrate. Claude is the only shipping `Runner` adapter today (shells out to `claude -p`), but the interface is deliberately generic — other coding agents and harnesses (Qwen Code, OpenHands, a local script, or whatever's next) are supported by design and planned as adapters, not locked to one vendor. These need a human actively driving the session in real time; syntropy is what keeps working after you close the laptop, whichever agent is doing the work. |
...
- **Pluggable runner and provider — other coding agents/harnesses are on the roadmap.** Claude ships today; the `Runner` interface (Qwen Code, OpenHands, a local script, or whatever's next) and the `Provider` interface (GitLab, GitHub) are both designed to take more adapters without touching the core state machine — supporting more agents and harnesses is a stated direction, not just a theoretical extension point.

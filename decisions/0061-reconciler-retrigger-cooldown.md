...
`Threshold` (default 30 minutes). ADR-0053's idempotency argument —
...
**3-minute cooldown vs. 30-minute stuck threshold**: the two knobs
answer different questions and are deliberately not the same value.
`Threshold` (30 minutes) asks "how long can a legitimately slow agent
...
30-minute detection threshold before a further stuck episode gets
picked up again.
